...
import { Newspaper, Mail, ChevronRight, ShieldAlert, Lock } from 'lucide-react';
import {
  loginWithPassword,
  persistAuthSession,
  apiErrorMessage,
...
export default function NewsLogin() {
  ...
  const from = location.state?.from || '/admin/news';
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data.user?.role !== ROLES.NEWS_ADMIN) {
        setError('Only News Administrators can sign in here.');
        return;
      }
        ...
        <div className="relative bg-white border border-[#C8E6C9] rounded-3xl p-8 shadow-xl shadow-green-900/10">
          ...
          <div className="text-center mb-10 relative z-10">
            <div className="w-16 h-16 bg-[#E8F5E9] rounded-2xl border border-[#C8E6C9] flex items-center justify-center mx-auto mb-6">
              <Newspaper className="w-8 h-8 text-[#2E7D32]" />
            </div>
            <h1 className="text-3xl font-black text-gray-900 uppercase tracking-wider mb-2">News Admin</h1>
            <p className="text-gray-600 text-sm font-medium">News Management Authentication</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="news.admin@university.edu"
                />
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full py-4 rounded-xl font-black uppercase tracking-widest text-sm flex items-center justify-center transition-all duration-300 ${
                loading
                  ? 'opacity-60 cursor-not-allowed bg-[#43A047] text-white'
                  : 'bg-gradient-to-r from-[#43A047] to-[#4CAF50] hover:shadow-lg hover:shadow-green-600/25 hover:-translate-y-0.5 text-white'
              }`}
            >
              {loading ? (
